...
export const placeBid = async (req, res) => {
    ...
    try {
        ...
        // Authorization check: User must be an admin, auctioneer, or the owner of the team
        if (req.user && req.user.role !== 'admin' && req.user.role !== 'auctioneer') {
            if (req.user.team_id !== teamId) {
                return res.status(403).json({ error: 'Unauthorized: You can only bid for your own team.' });
            }
        }
