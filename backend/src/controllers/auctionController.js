...
export const placeBid = async (req, res) => {
    ...
    try {
        ...
        // Authorization check: Only admins or the actual team owner can place a bid for this team
        if (!req.user || (req.user.role !== 'admin' && (req.user.role !== 'team_owner' || String(req.user.team_id) !== String(teamId)))) {
            return res.status(403).json({ error: 'Unauthorized to place bid for this team' });
        }
