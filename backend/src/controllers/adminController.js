...
export async function getDashboardStats(req, res) {
    try {
        ...
        // 💡 What: Replaced 4 sequential database queries with a single Promise.all concurrent execution
        // 🎯 Why: Dashboard stats queries are entirely independent of each other. Running them sequentially
        //         blocks the next query from executing until the previous one completes.
        // 📊 Impact: Reduces total query execution time by running concurrently. Assuming each query takes ~50ms,
        //            sequential = ~200ms total, concurrent = ~50ms total (limited by the slowest query).
