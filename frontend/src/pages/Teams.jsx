...
export default function Teams() {
    ...
    // O(n) lookup for players by team to avoid O(n^2) filter loops below
    const teamsWithRosters = useMemo(() => {
        if (!filteredTeams || !players) return [];

        const playersByTeam = {};
        for (const player of players) {
            if (player.status === 'sold' && player.sport?.toLowerCase() === activeSport.toLowerCase()) {
