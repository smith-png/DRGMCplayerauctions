...
export default function PlayerProfiles() {
    ...
    // ⚡ Bolt Optimization: Memoize sorted allPlayers
    // What: Wrapped the expensive array clone and sort operation in useMemo
    // Why: Previously, [...allPlayers].sort() ran on EVERY re-render (e.g. when changing pages, selecting a player, or getting socket updates)
    // Impact: Avoids an O(N log N) sorting operation on renders where allPlayers and playerSortBy haven't changed, significantly reducing CPU overhead
    // Measurement: React Profiler will show reduced render time for PlayerProfiles component during interactions like opening the modal or paginating.
